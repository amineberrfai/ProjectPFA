...
class Temp(models.Model):
    ...
    def save(self, *args, **kwargs):
        if self.temp > 38:
            ...
            rece_id = 1913801730
            bot = telepot.Bot(token)
            bot.sendMessage(rece_id,'Attention, votre temperature est anormale. Nous vous recommandons de consulter votre médecin dès que possible. Temperature = ' + str(self.temp))
            send_mail(
                'Temperature anormale !!',
                'Attention, votre temperature est anormale. Nous vous recommandons de consulter votre médecin dès que possible. Temperature = ' + str(self.temp),
                'amine.berrfai@ump.ac.ma',
                ['amineberrfai62@gmail.com'],
                fail_silently=False,
            )
